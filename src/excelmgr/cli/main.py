...
def combine(
    ...
    try:
        ...
        logger.error("combine_failed", error=str(exc))
        raise typer.Exit(code=2) from exc
    except Exception as exc:  # pragma: no cover - defensive guard
        logger.error("combine_crash", error=str(exc))
...
def split(
    ...
    try:
        ...
        logger.error("split_failed", error=str(exc))
        raise typer.Exit(code=2) from exc
    except Exception as exc:  # pragma: no cover - defensive guard
        logger.error("split_crash", error=str(exc))
        raise typer.Exit(code=1) from exc
...
def delete_cols(
    ...
    try:
        ...
        logger.error("delete_cols_failed", error=str(exc))
        raise typer.Exit(code=2) from exc
    except Exception as exc:  # pragma: no cover - defensive guard
